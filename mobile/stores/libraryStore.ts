...
export const useLibraryStore = create<LibraryState>((set, get) => ({
  ...
  likeTrack: async (track) => {
    set((s) => ({ likedTracks: [track, ...s.likedTracks.filter(t => t.video_id !== track.video_id)] }));
  ...
  addToRecent: async (track) => {
    set((s) => ({ recentlyPlayed: [track, ...s.recentlyPlayed.filter(t => t.video_id !== track.video_id)].slice(0, 50) }));
  ...
  loadFromStorage: async () => {
    try {
      ...
      set({ likedTracks: liked ? JSON.parse(liked) : [], recentlyPlayed: recent ? JSON.parse(recent) : [] });
