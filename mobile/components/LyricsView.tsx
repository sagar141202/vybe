      ...
      <View style={styles.center}>
        <View style={styles.unavailableCard}>
          ...
          <Text style={styles.unavailableSubtitle}>
            ...
            <Text style={styles.unavailableTrack}>"{title}"</Text>
          </Text>
    ...
    return (
      <ScrollView style={styles.scroll} contentContainerStyle={styles.plainContent} showsVerticalScrollIndicator={false}>
        <View style={styles.sourceBadge}>
          <LinearGradient colors={['rgba(167,139,250,0.15)', 'rgba(125,211,252,0.08)']} style={StyleSheet.absoluteFillObject} />
          <Text style={styles.sourceText}>📝 Plain Lyrics · {source}</Text>
        </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.sourceBadgeTop}>
        ...
        <Text style={styles.sourceTextTop}>✦ SYNCED · {lines.length} LINES</Text>
      </View>
        ...
        {lines.map((line, i) => {
          ...
          return (
            <TouchableOpacity key={i} onPress={() => {
              ...
              <Animated.View style={[styles.lineWrap, isActive && styles.lineWrapActive, { transform: [{ scale: lineAnims.current[i] || new Animated.Value(1) }] }]}>
                {isActive && <LinearGradient colors={['rgba(167,139,250,0.15)', 'rgba(125,211,252,0.08)']} style={StyleSheet.absoluteFillObject} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} />}
                {isActive && <View style={styles.activeDot} />}
                <Text style={[styles.lineText, isPast && styles.lineTextPast, isActive && styles.lineTextActive]}>{line.text}</Text>
              </Animated.View>
            </TouchableOpacity>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  loadingWrap: {
    padding: 32, borderRadius: 28, overflow: 'hidden',
    alignItems: 'center', borderWidth: 1.5,
    borderColor: 'rgba(167,139,250,0.2)', width: '100%',
  },
  loadingEmoji: { fontSize: 40, marginBottom: 12 },
  loadingText: { fontSize: 17, color: '#7C3AED', fontWeight: '700', marginBottom: 4 },
  loadingSubtext: { fontSize: 13, color: '#9CA3AF' },
  unavailableCard: {
    padding: 32, borderRadius: 28, overflow: 'hidden',
    alignItems: 'center', borderWidth: 1.5,
    borderColor: 'rgba(167,139,250,0.15)', width: '100%',
  },
  ...
  unavailableTrack: { color: '#7C3AED', fontWeight: '700' },
  unavailableTips: { gap: 8, width: '100%' },
  tipText: { fontSize: 13, color: '#9CA3AF', textAlign: 'center' },
  sourceBadge: {
    alignSelf: 'center', marginBottom: 20, paddingHorizontal: 16,
  ...
  sourceBadgeTop: {
    alignSelf: 'center', marginBottom: 12, paddingHorizontal: 16,
    paddingVertical: 6, borderRadius: 20, overflow: 'hidden',
  },
